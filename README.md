...
### 一条命令：先查退稿原因 → 剪片 → 上传 → 继续轮询（推荐）

已有 BV 与本地成片、希望**先看清退回说明再自动改片并替换**，且**再次退稿仍自动处理**时：

```bash
python bilibili_review.py BV1xxxxxxxxxx ./video_subs/yt_xxx_bilingual.mp4 --auto-repair
```

流程：查询并打印退回说明 → 解析时间段 → ffmpeg 剪片 → 替换分 P → 轮询审核（通过结束；再退则重复，上限见 `BILIBILI_REVIEW_MAX_REPLACE_ROUNDS`）。若当前为「审核中」，会先轮询，退回后再执行上述步骤。
